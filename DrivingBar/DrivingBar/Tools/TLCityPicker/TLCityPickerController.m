...
@interface TLCityPickerController () <CLLocationManagerDelegate, TLCityGroupCellDelegate, TLSearchResultControllerDelegate>{
...
-(void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation{
    ...
    [geoCoder reverseGeocodeLocation:newLocation completionHandler:^(NSArray *placemarks, NSError *error){
        for (CLPlacemark *placemark in placemarks) {
            ...
            NSLog(@"location city: %@",[test objectForKey:@"City"]);
           // NSLog(@"%@",test);
            //更新定位城市数据
            TLCity *city = nil;
            for (TLCity *item in self.cityData) {
                if ([item.cityName isEqualToString:[test objectForKey:@"City"]]) {
                    city = item;
                    break;
                }
            }
            [_localCityData removeAllObjects];
            [_localCityData addObject:city];
