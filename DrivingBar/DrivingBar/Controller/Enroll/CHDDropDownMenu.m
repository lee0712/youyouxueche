...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if ([super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.textL = [[UILabel alloc] initWithFrame:CGRectMake(18,0,300, cellHeight)];
        self.textL.font = [UIFont systemFontOfSize:14.0f];
...
@implementation CHDDropDownMenu
{
    ...
    BOOL isShow;
    UIView *bgView;
    void (^chdClickAction)(long section, long row);
}

- (void)initWithFrame:(CGRect)frame showOnView:(UIView*)view AllDataArr:(NSMutableArray*)arr showArr:(NSMutableArray *)showArr clickAction:(void (^)(long section, long row))clickAction
{
    if ([super initWithFrame:frame]) {
        self.AllDataArr = arr;
        self.showArr = showArr;
        chdClickAction = clickAction;
        if (!showArr) {
            self.showArr = arr;
        }
        ...
        for (int i=0; i<arr.count; i++) {
            ...
            [button setImage:[UIImage imageNamed:@"jiaxiao_choose_down"] forState:UIControlStateNormal];
           // button.imageView.transform = CGAffineTransformMakeRotation(M_PI);
            [button setTitleColor:RGB_BACKGROUND_COLOR_BLUE forState:UIControlStateNormal];
...
- (void)selectClum:(NSInteger)colum Row:(NSInteger)row
{
    ...
    [btn setImage:[UIImage imageNamed:@"jiaxiao_choose_down"] forState:UIControlStateNormal];
...
- (void)buttonClick:(chdButton*)button
{
    ...
    [button setImage:[UIImage imageNamed:@"jiaxiao_choose_up"] forState:UIControlStateNormal];
    [button setTitleColor:RGB_BACKGROUND_COLOR_BLUE forState:UIControlStateNormal];
    NSLog(@"chd click");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.isSelect) {
        cell.textL.textColor = RGB_BACKGROUND_COLOR_BLUE;
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    chdClickAction(currentSelect,indexPath.row);
}
/*
